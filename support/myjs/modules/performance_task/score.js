...
let studentTable =  $('#studentScoreTable').DataTable(
    {
        "processing": true,
            "ajax": `${base_url}/performance_task/topic/student_scoring_list`,
            "columns": [
                { "width": "70%", "data": "name" },
                { "width": "30%", "data": "score" }
            ],
    ...
    $(document).on('click','.btnScore', function (){
        ...
        $.get(`${base_url}/performance_task/topic/getStudent/${id}`, function(output)
        {
            console.log(output);
    ...
    $(document).on('click', '.btnScoreEdit', function(){
        let id = $(this).attr('id');
        $.get(`${base_url}/performance_task/topic/getScore/${id}`, function(output)
        {
            $.get(`${base_url}/performance_task/topic/getStudent/${output[0].student_id}`, function(outputs)
            {
    ...
    $('#scoreForm').submit(function(e){
        e.preventDefault();
        $.post(`${base_url}/performance_task/topic/save`, $(this).serialize(), function(succ)
        {
            if(succ.status)
            {
                Swal.fire(
