...
class Student extends MY_Controller 
{
    ...
    function student_list()
    {
        ...
        foreach ($list as $t) 
        {
            ...
            $row['sex'] = $t->sex;
            $overall = $this->getOverAllGrade($t->student_id);
            $tt='';
            if($overall < 75)
            {
            ...
            $row['grade'] = $tt;
            $row['action'] = "<a href='".base_url('student/student_info/index/'.$t->student_id)."' class='btn btn-primary btn-sm btnStudentInfo'><i class='fa fa-user'></i> Student Info</a>";
            $data[] = $row;
        }
    ...
    function getOverAllGrade($student_id)
    {
        $grade =0;
        $subjects = array('Filipino','English','Mathematics','Science','AP','ESP','EPP','MAPEH');
        $hundred =0;
        for($i = 0; $i < count($subjects); $i++)
        {
            $num = $this->totalGrade($subjects[$i], $student_id);
        ...
        return $this->transmute_grade(($grade / $hundred) * 100);
